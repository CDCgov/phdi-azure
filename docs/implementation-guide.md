...
- [PHDI Azure Implementation Guide](#phdi-azure-implementation-guide)
    ...
    - [Implementing the PHDI Azure Pipelines](#implementing-the-phdi-azure-pipelines)
        ...
        - [Step 3: Run the GitHub Deployment Workflow](#step-3-run-the-github-deployment-workflow)
        - [Step 4: Run an Hl7v2 vaccination message through the pipeline](#step-4-run-an-hl7v2-vaccination-message-through-the-pipeline)
        - [Basic guide to FHIR Server REST calls](#basic-guide-to-fhir-server-rest-calls)
...
- Bearer token (autoexpires and may need regeneration every 60~90 mins)

    - Generate Token:

      `az account get-access-token --resource=https://phdi-dev-fhir-server.azurehealthcareapis.com --query accessToken --output tsv`

- Api tool - Postman, Insomnia, or curl. Make sure to set up your auth header using the generated token.
...
`{{fhirBaseUrl}}/{{Resource Name}}?_include={{Resource Name}}:{{matching reference type}}`

Examples:

    GET all Observation Resources AND the Patient resource referenced within:  
    `{{fhirBaseUrl}}/Observation?_include=Observation:subject`
    
    GET a specific Observation Resource AND the Performer referenced within:  
    `{{fhirBaseUrl}}/Observation?_id=<id>&_include=Observation:performer`
    
    GET a Claim resource by ID AND the Provider referenced within:  
    `{{fhirBaseUrl}}/Claim?_id=<id>&_include=Claim:provider`

####_revinclude
GET a resource AND resource(s) referencing that resource

Template to join resources using _revinclude:  
`{{fhirBaseUrl}}/{{Resource Name}}?_include={{Referencing Resource Name}}:{{matching reference type}}`
...
Examples:
    ...
    `{{fhirBaseUrl}}/Practitioner?_id=<id>&_revinclude=Encounter:practitioner`
